...
function circleBorderWalls(w, h) {
  // Approximate a larger ellipse border to look like a tall circular arena.
  const walls = [];
  const cx = (w - 1) / 2;
  const cy = (h - 1) / 2;
  // Slightly larger radii so the "circle" reaches further up and down.
  const rx = (w - 1) / 2;
  const ry = h / 2; // increase vertical radius for a taller circle
  ...
  for (let x = 0; x < w; x++) {
    for (let y = 0; y < h; y++) {
      ...
      const dy = (y - cy) / ry;
      const d = dx * dx + dy * dy;
      // Keep cells that are near the ellipse radius (between inner and outer thresholds).
      // Widen the band so the border looks thicker, especially on top and bottom.
      if (d > 0.7 && d < 1.25) {
        walls.push([x, y]);
      }
    }
